...
open class SkyFloatingLabelTextFieldWithIcon: SkyFloatingLabelTextField {
    ...
    @IBInspectable open var iconHeight:CGFloat = 20 {
        didSet {
            self.updateFrame()
        }
    }
    ...
    */
    ...
    fileprivate func createIconLabel() {
        ...
        iconImage.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin]
   ...
   override open func titleLabelRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        if editing {
            return CGRect(x: iconWidth, y: 0, width: bounds.size.width - iconWidth, height: titleHeight())
        }
        return CGRect(x: iconWidth, y: titleHeight(), width: bounds.size.width - iconWidth, height: titleHeight())
    }
    ...
    fileprivate func updateFrame() {
        ...
        if isLTRLanguage {
            ...
            self.iconImage.frame = CGRect(x: 0, y: self.bounds.size.height - iconHeight - iconMarginBottom, width: iconWidth, height: iconHeight)
            ...
            self.iconImage.frame = CGRect(x: textWidth - iconWidth , y: self.bounds.size.height - iconHeight - iconMarginBottom, width: iconWidth, height: iconHeight)
        }
    }
}
